...
#If we don't change the gyroscope sample rate to 1khz and we try to get 20 hz.
#smplrt_div will be 399, which is higher than the maximum storage that the smplrt_div register have.
#So we must to change it first to 1khz
...
while True:
    #Aquí es donde ocurre el polling, tenemos que usar el bus i2c cada iteración para comprobar si hay una nueva muestra, o sea, ocupamos el bus i2c
    if int(i2c.readfrom_mem(device[0],58, 1)[0]) == 1:
        raw = i2c.readfrom_mem(device[0], 0x3b, 14)
        raw2val(raw, offsets)